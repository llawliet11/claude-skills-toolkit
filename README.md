...
| [`nf-memoria`](./nf-memoria/) | Capture the active session's in-flight state into a structured handoff memory file so a future session of the SAME repo resumes without re-discovery. Resolves the target memory folder via a four-step cascade (env, project settings, user settings, binary default), writes canonical frontmatter (type=project, project, cwd, tags=[handoff, in-progress]), and runs an auxiliary auto-memory pass that captures durable user/feedback/project/reference learnings from the same conversation. | _pending_ |
